...
export const getTimeDuration = (startTime, endTime,resumeTime,pauseTime) => {
  ...
  if (typeof resumeTime === 'string' || typeof resumeTime === 'number') {
    resumeTime = new Date(resumeTime)
  }
  if (typeof pauseTime === 'string' || typeof pauseTime === 'number') {
    pauseTime = new Date(pauseTime)
  }
  let durationInSecond 
if(resumeTime){
   durationInSecond = Math.round((endTime - startTime - (resumeTime - pauseTime)) / 1000)
}
else
{
   durationInSecond = Math.round((endTime - startTime) / 1000)
}
